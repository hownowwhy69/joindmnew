...
class ChatRow:
    ...
    ub_wid: int = 0            # userbot Saved Messages me welcome msg id (FORWARD)
    created_at: int = 0
...
class Database:
    ...
    # ── userbot dedup: claim + actually-sent tracking ──
    def mark_sent(self, chat_id: int, user_id: int):
        """Welcome ACTUALLY user ko pahuncha (worker success par) — 3 din
        double-send nahi hoga."""
        self._db["dedup"].update_one(
            {"key": f"s:{chat_id}:{user_id}"},
            {"$set": {"ts": time.time()}}, upsert=True)

    def was_sent(self, chat_id: int, user_id: int) -> bool:
        # Sirf POLLER/RECOVERY ke liye: jise welcome mil chuka hai (24h), use
        # poller baar-baar na bheje. REAL join request hamesha force bhejta
        # hai (jab-jab). Approve/Decline par clear → agla request = naya msg.
        d = self._db["dedup"].find_one({"key": f"s:{chat_id}:{user_id}"})
        return bool(d and time.time() - (d.get("ts") or 0) < 24 * 3600)

    def release_claim(self, chat_id: int, user_id: int):
        self._db["dedup"].delete_one({"key": f"w:{chat_id}:{user_id}"})

    def clear_request(self, chat_id: int, user_id: int):
        """User approve/decline hua — sent + claim dono clear. Uske agle
        join request par WAPAS welcome jayega (jab-jab request = tab-tab DM)."""
        self._db["dedup"].delete_one({"key": f"s:{chat_id}:{user_id}"})
        self._db["dedup"].delete_one({"key": f"w:{chat_id}:{user_id}"})

    def clear_requests_chat(self, chat_id: int):
        """Approve-ALL ke baad: us chat ke saare users ke sent/claim clear."""
        self._db["dedup"].delete_many({"key": {"$regex": f"^s:{chat_id}:"}})
        self._db["dedup"].delete_many({"key": {"$regex": f"^w:{chat_id}:"}})
    ...
    def update_chat(self, chat_id: int, **fields):
        allowed = {"welcome_text", "media_file_id", "media_kind", "buttons",
                   ...
                   "ub_enabled", "ub_admin", "ub_bot_both", "ub_wid"}
...
def _ub_kb(row: ChatRow):
    ...
    main bot ke hote hain). Telethon 1.44: InlineKeyboardMarkup hat gaya —
    ab KeyboardButtonUrl ki list-of-lists return karo (send_message khud
    ReplyInlineMarkup bana leta hai)."""
    try:
        rows = []
        for brow in row.button_rows():
            ...
            for b in brow:
                if isinstance(b, dict) and b.get("url"):
                    btns.append(tltypes.KeyboardButtonUrl(
                        text=str(b.get("text", ""))[:63],
                        url=str(b["url"])[:200]))
            if btns:
                rows.append(btns)
        return rows or None
    except Exception as e:
        _ub_log(f"kb build err: {str(e)[:60]}")
        return None
...
async def ub_send(uid: int, text: str, ents: list | None = None,
                  kb=None, tries: int = 0, cid: int | None = None,
                  kind: str = "txt", msg_id: int = 0,
                  peer=None) -> bool:
    """Welcome message ko spam-safe QUEUE me daalo (worker real send karta hai).
    kind="fwd" → userbot SAVED welcome message ko FORWARD karega (original
    media + text — copy-paste nahi).
    peer → InputPeerUser(user_id, access_hash) — BINA access_hash Telegram
    galti se "USER_DELETED" bol deta hai (enumeration-protection)!
    """
    if not ub_ready():
        return False
    q = UB.get("queue")
    ...
    await q.put({"uid": uid, "text": text, "ents": ents or [],
                 "kb": kb, "tries": tries, "cid": cid,
                 "kind": kind, "msg_id": int(msg_id or 0), "peer": peer})
    return True
...
async def ub_worker():
    ...
    while True:
        ...
        while True:
            ...
            break
        cl = UB.get("client")
        act = "fwd" if (item.get("kind") == "fwd" and item.get("msg_id")) else "txt"
        try:
            target = item.get("peer") or item["uid"]
            if act == "fwd":
                # ORIGINAL message forward (media + text + premium emojis)
                await cl.forward_messages(target, messages=item["msg_id"],
                                          from_peer="me")
            else:
                await cl.send_message(target, item["text"],
            ...
            if item.get("cid"):
                try:
                    db.mark_sent(item["cid"], item["uid"])
                except Exception:
                    pass
            ...
            emsg = str(e)
            # forward nahi chala (media delete / koi dikkat) → text version bhejo
            if act == "fwd":
                try:
                    await cl.send_message(
                        item.get("peer") or item["uid"],
                        item["text"] or "🎉 Welcome!",
                        formatting_entities=item["ents"] or None,
                        parse_mode=None, link_preview=False)
                    UB["sent_total"] += 1
                    UB["sent_day"] += 1
                    ...
                    if item.get("cid"):
                        try:
                            ...
                            pass
                    _ub_log(f"fwd fail → text fallback OK: {item['uid']}")
                    continue
                except Exception as e2:
                    emsg = str(e2)
            _ub_log(f"send fail {item['uid']}: {emsg[:100]}")
            # deleted user → silent skip (owner ko spam nahi)
            if "deleted" in emsg.lower():
                _ub_log(f"user deleted: {item['uid']}")
                if item.get("cid"):
                    try:
                        db.mark_sent(item["cid"], item["uid"])
                    except Exception:
            ...
            try:
                ...
                while True:
                    ...
                    if not imps:
                        break
                    for imp in imps:
                        uid_b = getattr(imp, "user_id", 0)
                        if not uid_b or uid_b in seen_b:
                            continue
                        seen_b.add(uid_b)
                        await _ub_deliver(row.chat_id, uid_b)
                        got += 1
                        total += 1
                    if len(imps) < 100:
                        break
...
async def _ub_approve_one(cid: int, uid: int, approve: bool) -> tuple[bool, str]:
    ...
    try:
        ...
        db.clear_request(cid, uid)          # ✔ jab-jab: agli request = naya welcome
...
async def _ub_approve_all(cid: int) -> tuple[int, str]:
    ...
    try:
        ...
        db.clear_requests_chat(cid)         # ✔ jab-jab: unki agli request = naya welcome
    except Exception as e:
...
class _NamedIO(io.BytesIO):
    """Telethon ko file ka naam do (photo.jpg → photo, video.mp4 → video)."""
    def __init__(self, data, name):
        super().__init__(data)
        self.name = name


def _aiogram_media_of(message):
    """aiogram message se (kind, file_id, filename) nikalo. Nahi to (None,None,None)."""
    ...
    if photo:
        sizes = photo if isinstance(photo, (list, tuple)) else [photo]
        fid = sizes[-1].file_id if sizes else ""
        return ("photo", fid, "welcome.jpg")
    if video:
        return ("video", video.file_id, "welcome.mp4")
    if anim:
        return ("animation", anim.file_id, "welcome.gif")
    if doc:
        return ("document", doc.file_id,
                getattr(doc, "file_name", None) or "welcome.file")
    if audio:
        return ("audio", audio.file_id,
                getattr(audio, "file_name", None) or "welcome.mp3")
    return (None, None, None)


async def _ub_save_welcome(cid: int, message) -> tuple[bool, str]:
    """Owner ka bheja message (text ya text+media) userbot ke Saved Messages
    me save karo → uski id store. Userbot users ko ise FORWARD karega
    (copy-paste nahi) — original media + premium emojis ke saath."""
    if not ub_ready():
        return False, "⚠️ Userbot login nahi hai"
    cl = UB["client"]
    ...
    try:
        ents_tl = _ub_convert_entities(getattr(message, "entities", None) or [])
    except Exception:
        ents_tl = []
    kind, fid, fname = _aiogram_media_of(message)
    if kind is None and not text:
        return False, "📝 Kuch bhejo — text ya photo/video/GIF (caption ke saath)"
    try:
        if kind and fid:
            buf = io.BytesIO()
            ...
            data = buf.getvalue()
            if not data:
                return False, "❌ Media download nahi hua — dobara bhejo"
            res = await cl.send_file(
                "me", _NamedIO(data, fname),
                caption=text or "",
                formatting_entities=ents_tl or None,
                parse_mode=None,
                force_document=(kind == "document"),
                supports_streaming=(kind in ("video", "animation")))
        else:
            res = await cl.send_message("me", text,
                                        formatting_entities=ents_tl or None,
                                        parse_mode=None, link_preview=False)
        wid = int(getattr(res, "id", 0) or 0)
        if not wid:
            return False, "❌ Message id nahi mili — dobara try karo"
        db.update_chat(cid, ub_wid=wid)
        _ub_log(f"✅ userbot welcome SAVED (id {wid}) — chat {cid}")
        return True, (f"✅ <b>Userbot welcome SET!</b>\n"
                      "Yehi message ab <b>har naye join-request user ko "
                      "FORWARD</b> hoga — text + media + premium emojis ke "
                      "saath (copy-paste nahi).")
    except Exception as e:
        _ub_log(f"save welcome fail: {str(e)[:80]}")
        return False, f"❌ Save nahi hua: <code>{esc(str(e)[:120])}</code>"
...
async def _ub_test_dm(uid: int) -> str:
    ...
    try:
        ...
        for r in db.get_chats():
            if r.ub_enabled and r.ub_wid:
                trow = r
                break
        if trow is not None and hasattr(UB["client"], "forward_messages"):
            await UB["client"].forward_messages(uid, messages=trow.ub_wid,
                                                from_peer="me")
            _ub_log(f"🧪 Test DM (saved welcome forward) -> {uid} OK")
            return "OK"
        trow = None
        for r in db.get_chats():
            if r.ub_enabled:
...
async def _ub_deliver(cid: int, uid: int, force: bool = False) -> None:
    """Ek user ko welcome bhejo (rate-limited queue ke through).
    force=True (real join-request update se) → HAMESHA bhejo, chahe pehle
    bhi ja chuka ho (jab-jab join = tab-tab msg).
    force=False (poller/backfill recovery) → sirf jisko abhi tak nahi gaya."""
    if not ub_ready():
    ...
    if not row.ub_enabled or not row.ub_admin:
        ...
        return
    # JAB-JAB RULE (final — koi blocking nahi):
    # • HAR join-request update (force) → HAMESHA DM. Chahe user ne 1 min
    #   pehle bheja ho, 10 min pehle, ya pehle bhi message mil chuka ho —
    #   koi record, koi skip, koi decline ki zaroorat NAHI.
    # • Poller/backfill (recovery, force=False) hi sirf unko bhejta hai
    #   jinko abhi tak message NAHI gaya — taaki pending users ko har 60s
    #   baar-baar na bheje (spam-safe).
    # • 15-min claim sirf recovery path ke liye hai; force path ise bypass
    #   karta hai (par fir bhi mark karta hai taaki poller duplicate na bheje).
    if not force:
        if db.was_sent(cid, uid):
            _ub_log(f"SKIP {uid} -> {cid}: welcome already mil chuka (recovery skip)")
            return
        if not db.claim_welcome(cid, uid, ttl=900):
            _ub_log(f"SKIP {uid} -> {cid}: duplicate — queue me already hai")
            return
    else:
        db.claim_welcome(cid, uid, ttl=900)   # poller ke liye mark, khud kabhi nahi rukta
    # user info + ACCESS_HASH (zaroori!): ID se bhejne par Telegram "deleted"
    # bol deta hai — isliye full User/peer resolve karo
    first, last, uname = f"User {uid}", "", ""
    peer = None
    try:
        uu = UB["pending_cache"].get(cid, {}).get(uid)
        if uu is None:
            users = await UB["client"](functions.users.GetUsersRequest(
                id=[tltypes.InputUser(user_id=uid, access_hash=0)]))
            uu = users[0] if users else None
            if uu is not None:
                UB["pending_cache"].setdefault(cid, {})[uid] = uu
        if uu is not None:
            first = getattr(uu, "first_name", "") or ""
            last = getattr(uu, "last_name", "") or ""
            uname = getattr(uu, "username", "") or ""
            ah = int(getattr(uu, "access_hash", 0) or 0)
            if ah:
                peer = tltypes.InputPeerUser(user_id=uid, access_hash=ah)
    except Exception:
        pass
    if peer is None:
        # importers response me full User (access_hash guaranteed) hota hai
        try:
            ent = UB["entities"].get(cid)
            if ent is not None:
                r = await UB["client"](functions.messages.GetChatInviteImportersRequest(
                    peer=ent, offset_date=datetime.now(timezone.utc),
                    offset_user=tltypes.InputUserEmpty(),
                    requested=True, limit=100))
                for u2 in getattr(r, "users", None) or []:
                    if int(getattr(u2, "id", 0) or 0) == uid:
                        UB["pending_cache"].setdefault(cid, {})[uid] = u2
                        ah2 = int(getattr(u2, "access_hash", 0) or 0)
                        if ah2:
                            peer = tltypes.InputPeerUser(user_id=uid,
                                                         access_hash=ah2)
                        break
        except Exception:
            pass
    if peer is None:
        _ub_log(f"WARN {uid}: access_hash resolve nahi hua — raw id se try")
    ...
    try:
        ents_tl = _ub_convert_entities(ents)
    except Exception as e:
        _ub_log(f"entity convert err {uid}: {str(e)[:60]}")
        ents_tl = []
    if row.ub_wid:
        # SAVED userbot welcome → FORWARD (original media + text)
        ok = await ub_send(uid, text, ents_tl, None, cid=cid,
                           kind="fwd", msg_id=row.ub_wid, peer=peer)
    else:
        ok = await ub_send(uid, text, ents_tl, _ub_kb(row), cid=cid,
                           peer=peer)
...
async def _ub_on_join(ev):
    ...
    if isinstance(u, tltypes.UpdateBotChatInviteRequester):
        ...
        await _ub_deliver(cid, uid, force=True)


def _ub_seen_event(cid: int, uid: int, key) -> bool:
    """Same update (latest_date) ke duplicate (reconnect/replay) ko 45s roko.
    Nayi request = naya latest_date → hamesha fresh message."""
    k = f"{cid}:{uid}:{key or 0}"
    now = time.time()
    seen = UB.setdefault("seen_events", {})
    if k in seen and now - seen[k] < 45:
        return True
    if len(seen) > 8000:
        seen.clear()
    seen[k] = now
...
async def _ub_poll_chat(cid: int, fresh_minutes: int = 30) -> int:
    """Fresh join-request users (30 min ke andar) hi queue me daalo.
    PURANE pending users ko NAHI chhedta — sirf abhi ki requests."""
    if not ub_ready():
        return 0
    if not ub_ready():
        return 0
    ...
    try:
        ...
        r = await cl(functions.messages.GetChatInviteImportersRequest(
            ...
            requested=True, limit=100))
        imps = list(getattr(r, "importers", None) or [])
        for u3 in getattr(r, "users", None) or []:
            UB["pending_cache"].setdefault(cid, {})[
                int(getattr(u3, "id", 0) or 0)] = u3
        seen = set()
        cutoff = time.time() - (int(fresh_minutes) * 60)
        fresh = 0
        for imp in imps:
            uid = getattr(imp, "user_id", 0)
            if not uid or uid in seen:
            ...
            t = d.timestamp() if isinstance(d, datetime) else (float(d or 0))
            if t and t < cutoff:
                continue          # purana pending → SKIP (sirf abhi ki requests)
            fresh += 1
            await _ub_deliver(cid, uid)
        if imps:
            _ub_log(f"🔎 poll {cid}: {fresh} fresh (purane {len(imps) - fresh} skip)")
        return fresh
    except Exception as e:
...
async def _ub_on_pending(ev):
    ...
    if isinstance(u, tltypes.UpdatePendingJoinRequests):
        ...
        rrs = list(getattr(u, "recent_requesters", None) or [])
        key_date = int(getattr(u, "latest_date", 0) or 0)
        _ub_log(f"📥 join-request update: {n} pending, naye: {rrs[:8]} -> {cid}")
        # JAB-JAB: SIRF naye requesters ko FORCE bhejo (har join = har message)
        # Purane pending users ko YAHAN se kabhi message NAHI jayega.
        sent_now = 0
        for uid in rrs[:30]:
            if not uid:
                continue
            if _ub_seen_event(cid, uid, key_date):
                continue
            await _ub_deliver(cid, uid, force=True)
            sent_now += 1
        if sent_now:
            _ub_log(f"📨 fresh: {sent_now} user(s) ko welcome queue me")
        # halka poll (30 min fresh): users + access_hash cache, aur koi
        # fresh request update me na ho to bhi pakde (duplicate claim se bacha)
        try:
            await _ub_poll_chat(cid, fresh_minutes=30)
        except Exception:
            pass
...
def ub_status_text() -> str:
    ...
    try:
        ...
        for r in rows:
            if r.ub_enabled or r.ub_admin:
                ...
                if r.ub_admin:
                    n_admin += 1
                mark = "✅" if r.ub_enabled else "⛔"
                wm = " 📨" if r.ub_wid else ""
                chat_lines.append(f"   {mark} {esc(r.title or str(r.chat_id))}{wm}")
    except Exception:
        pass
    chat_list = ("\n" + "\n".join(chat_lines[:10])) if chat_lines \
        else " (koi linked chat nahi)"
    if len(chat_lines) > 10:
        chat_list += f"\n   … aur {len(chat_lines) - 10} (📨 = welcome set)"
    ...
    return ("🤖 <b>Userbot: LOGIN OK</b>\n"
            ...
            f"(ON: {n_on} | ADMIN: {n_admin}):{chat_list}\n"
            f"🗂️ Spam guard: "
...
def panel_text(row: ChatRow) -> str:
    ...
    return (
        ...
        + (f"🤝 Bot bhi bheje: ✅ ON (userbot + bot dono)\n"
           if (row.ub_enabled and row.ub_bot_both) else "")
        + (f"🤖 Userbot msg: ✅ SET (forward)\n" if row.ub_wid else "")
...
def panel_kb(row: ChatRow) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📨 Set userbot welcome msg" + (" ✅" if row.ub_wid else ""),
                              callback_data=f"ubwel:{cid}")],
...
async def on_private_message(message: Message):
    ...
    # ── waiting for media ───────────────────────────────────────────────────
    # ── USERBOT WELCOME SET (text/media → saved → FORWARD) ────────────
    if flow == "ubwel":
        chat_id = int(payload)
        ok, res = await _ub_save_welcome(chat_id, message)
...
async def on_callback(query: CallbackQuery):
    ...
    if data.startswith("ubwel:"):
        ...
        if not ub_ready():
            ...
            return
        await query.answer()
        STASH[query.from_user.id] = ("ubwel", str(cid))
        try:
            await msg.edit_text(
                f"📨 <b>Userbot welcome message set karo</b> — "
                f"<b>{esc(row.title)}</b>\n\n"
                "Abhi yahan wahi message bhejo jo users ko jana chahiye:\n"
                "• <b>Text</b> likho (premium emojis ke saath) ya\n"
                "• <b>Photo / Video / GIF + caption</b> ek saath bhejo\n\n"
                "Userbot ise <b>FORWARD</b> karega (copy-paste nahi) — original "
                "media + premium emojis sab rahenge. Har naye join-request user "
                "ko yehi message jayega. (/cancel se band)")
        except Exception:
            try:
                await msg.answer(
                    f"📨 <b>Userbot welcome message set karo</b> — "
                    f"<b>{esc(row.title)}</b>\n\n"
                    "Abhi yahan message bhejo — text ya photo/video/GIF + caption. "
                    "Userbot ise <b>FORWARD</b> karega.")
            except Exception:
                pass
        return
