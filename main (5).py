                ...
                while True:
                    ...
                    try:
                        offset_date = datetime.fromtimestamp(
                            int(getattr(last, "date", 0) or 0))
                    except Exception:
                        offset_date = None
                    offset_user = tltypes.InputUser(
                        user_id=int(getattr(last, "user_id", 0) or 0),
                        access_hash=0)
                if got:
                    chats += 1
...
async def _ub_test_dm(uid: int) -> str:
    ...
    try:
        ...
        await UB["client"].send_message(
            uid, text, entities=_ub_convert_entities(ents) or None,
            parse_mode=None, link_preview=False)
...
async def _ub_poll_chat(cid: int) -> int:
    ...
    try:
        cl = UB["client"]
        r = await cl(functions.messages.GetChatInviteImportersRequest(
            peer=ent, offset_date=None, offset_user=None,
            requested=True, limit=100))
        imps = list(getattr(r, "importers", None) or [])
        for imp in imps:
