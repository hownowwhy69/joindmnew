...
async def ub_send(uid: int, text: str, ents: list | None = None,
                  kb=None, tries: int = 0, cid: int | None = None) -> bool:
...
async def _ub_poll_chat(cid: int) -> int:
    ...
    try:
        ...
        if imps:
            _ub_log(f"🔎 poll {cid}: {len(seen)} pending user(s)")
        return len(seen)
    except Exception as e:
...
async def _ub_on_pending(ev):
    ...
    if isinstance(u, tltypes.UpdatePendingJoinRequests):
        ...
        _ub_log(f"📥 join-request update: {n} pending, naye: {rrs[:5]} -> {cid}")
        # JAB-JAB: naye requesters ko FORCE bhejo (har join = har message)
        for uid in rrs[:20]:
            await _ub_deliver(cid, uid, force=True)
        # recovery poll: purane pending jinko message nahi gaya
