    ...
    while True:
        ...
        try:
            await cl.send_message(item["uid"], item["text"],
                                  entities=item["ents"] or None,
                                  parse_mode=None, link_preview=False,
                                  reply_markup=item["kb"])
            UB["sent_total"] += 1
            UB["sent_day"] += 1
...
async def _ub_finalize_login(cl, me, phone: str):
    ...
    try:
        ...
        _ub_log("join-request handler registered")
    except Exception as e:
        _ub_log(f"handler register fail: {e}")
...
async def _ub_on_join(ev):
    """MTProto join request (UpdateBotChatInviteRequester) → userbot se
    welcome (BINA approve kiye) + owner ko notify."""
    try:
    ...
    if not uid or not cid:
        return
    row = db.get_chat(cid)
    if row is None or not row.ub_enabled or not row.ub_admin:
        return
    if not db.claim_welcome(cid, uid):
        return                      # main bot bhej chuka hai — duplicate nahi
    ...
    status = ("✅ welcome bheja ja raha hai (userbot se)"
              if ok else "⚠️ welcome queue me nahi ja paya")
    _ub_log(f"join: {name} -> {row.title} [{status}]")
...
async def _ub_bind(text: str, owner_uid: int) -> tuple[str, ChatRow | None]:
    ...
    if m:
        try:
            r = await cl(functions.messages.CheckChatInviteRequest(hash=m.group(1)[1:]))
        ...
        if text.lstrip("-").isdigit():
            cid = int(text)
            if cid < 0:
                mt = -(cid + 1000000000000)
                try:
                    rs = await cl(functions.channels.GetChannelsRequest(
                        id=[tltypes.InputChannel(channel_id=mt, access_hash=0)]))
                    chat = rs.chats[0] if rs.chats else None
                except Exception:
                    return ("❌ Is ID se resolve nahi hua (access hash nahi hota) — "
                            "invite link ya @username bhejo", None)
            else:
    ...
    try:
        perms = await cl.get_permissions(chat)
    except Exception as e:
        return f"❌ Permissions check nahi hua: {esc(str(e)[:80])}", None
    if not getattr(perms, "is_admin", False):
        return (f"❌ <b>{esc(getattr(chat, 'title', '?') or '?')}</b> me userbot "
                "ADMIN nahi hai — usko admin banao (welcome tabhi jayega)", None)
...
def ub_status_text() -> str:
    ...
    return ("🤖 <b>Userbot: LOGIN OK</b>\n"
            ...
            f"(total {UB['sent_total']})\n"
            f"⏳ Pending queue: {UB['queue'].qsize() if UB['queue'] else 0}\n"
            f"⏱ Gap: {UB_MIN_INTERVAL:.0f}s/msg | cap: {UB_DAILY_CAP}/day\n"
            f"🗂️ Spam guard: "
            f"{'ACTIVE (waiting)' if time.monotonic() < UB['flood_until'] else 'safe ✅'}")
...
async def _process_join(row: ChatRow, cjr: ChatJoinRequest, b: Bot):
    ...
    # ── USERBOT MODE: userbot (MTProto) hi welcome bhejega ──
    if row.ub_enabled and row.ub_admin and ub_ready():
        # userbot hi welcome bhejega — main bot sirf owner ko notify karega
        JOIN_LOG.append({
            "ts": time.time(), "user": name, "uid": user.id,
            "chat": row.title or str(row.chat_id), "chat_id": row.chat_id,
            "status": "ub", "auto": approved,
            "bot": "userbot(skip)"})
        await _notify_owner_join(row, cjr, name, username, approved, "ub")
        return
...
async def cmd_testemoji(message: Message):
    ...
    if not is_super_admin(message.from_user.id):
        await message.answer("⛔ Sirf super-admin ise use kar sakta hai.")
        return
